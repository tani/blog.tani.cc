...
推論規則としては、以下のような公理、カット、および論理規則（$\backslash L, \backslash R, / L, / R$）を持ちます。ここで、各記号は以下のような役割を持ちます。

- $\Gamma, \Delta, \Sigma$：論理式の有限の並び（文脈）を表します。
- $\alpha, \beta, \gamma$：個別の論理式（型）を表します。
- $\Rightarrow$：左辺の並びから右辺の型が導出可能であることを示すシーケントです。
...
さて、ここからが本題です。従来のランベック計算は有限の証明木しか持ちませんでしたが、ここに 循環証明 (Cyclic Proof) の枠組みを導入します。循環証明とは、証明木の中にループ（バックリンク）を許容することで、無限に続く証明構造を有限のグラフとして表現する手法です。今回提案する体系 $L\mu$ では、通常のランベック計算に、再帰的な定義を扱うための 最小不動点 ($\mu$) と、文脈に応じた選択（分岐）を行うための 連言 ($\land$) を追加しました。
...
> 注：　発表中では連言の導入が必要であると判断し話しましたが、その後、
> 循環証明体系の専門家との議論により必要ではないと判明しました。ここに訂正いたします。
> なお、余計な拡張というだけであり、真性の損失はないことを付記いたします。
